...
function LoadConfig () {
	...
	Config.MinGameTime = 400;		//6,40 minuten/minutes
	Config.MaxGameTime = 7200;      // 7200 2Stunden/hours original lower to 4200 1Stunde 10Minuten bot questet lvlt act1-act5 wenn kein heaylth disc  but buged in den of evil mit 2playern / messen der durchschnittsgamezeit
	...
	Config.PublicMode = 3 // 1 = invite and accept, 2 = accept only, 3 = invite only, 0 = disable. //test if workinfg with sololvling script

	// test if working with sololvling script (add: seems to work with sololvling script)
	Config.Leader = "LeaderNAme"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper) //  WICHTIG LEADER CHARNAME KEI NPROFILNAME
	Config.QuitList = ["QuitersName", "QuitersName"]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"];
	...
	Config.AutoMule.Exclude = [
		...
		"[name] == jahrune # # [MaxQuantity] == 1",	
		"[Name] == AmnRune # # [MaxQuantity] == 1",			//original start  without these lines
		"[Name] == RalRune # # [MaxQuantity] == 1",
		"[Name] == MalRune # # [MaxQuantity] == 1",
		"[Name] == IstRune # # [MaxQuantity] == 1",			
		"[Name] == OhmRune # # [MaxQuantity] == 1",			// end without
